...
# Consume de environments ignoring if they were executed already
...
class CEXP(object):
    ...
    def __init__(self, jsonfile, params=None, iterations_to_execute=0, sequential=False,
        ...
        # TODO: this goes out with the merge
        ...
        self._jsonfile_path = os.path.join(*jsonfile.split(os.sep)[:-1])
        ...
        if "COIL_DATASET_PATH" not in os.environ and self._params['save_dataset']:
            raise ValueError("COIL_DATASET_PATH not defined, set the place where the dataset is going to be saved!")
    ...
    def __iter__(self):
        ...
        if self._execute_all:
            ...
            for env_name in self._environments.keys():
                ...
                if env_name in Environment.number_of_executions.keys():
                    repetitions_rem = max(0, repetitions - Environment.number_of_executions[env_name])
        ...
        # These two modes ignore the repetitions parameter and just keep executing.
            ...
            return iter([self._environments[random.choice(list(self._environments.keys()))]
                         for _ in range(self._iterations_to_execute)])
